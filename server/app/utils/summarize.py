import json
from app.schemas.models import ChamberSummary, DistrictMap, FederalCache, FederalData, FederalSummary, LegislativeSummary, SenatorMap, StateCache, StateCongressionalSummary, StateData, StateMapData, StateSummary
...
async def Summarize_State_Gov_Data(state_abbr: str) -> StateData:
    ...
    return StateData(
        summary = summary,
        map = StateMapData(
            senate = stateSenateMap,
            house = stateHouseMap,
            congressional = congressionalMap
        )
    )
